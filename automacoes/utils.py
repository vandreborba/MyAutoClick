...
def gerar_e_salvar_chave_criptografia():
    """
    Gera uma chave Fernet, salva ofuscada no arquivo oculto e a retorna em base64.
    """
    chave_base64 = Fernet.generate_key()
    hash_maquina = obter_hash_maquina()
    chave_bytes_puros = base64.urlsafe_b64decode(chave_base64)
    chave_ofuscada = bytes([b ^ hash_maquina[i % len(hash_maquina)] for i, b in enumerate(chave_bytes_puros)])
    with open(CAMINHO_ARQUIVO_CHAVE, 'wb') as arquivo:
        arquivo.write(chave_ofuscada)
    return chave_base64
...
def carregar_chave_criptografia():
    """
    Carrega a chave Fernet do arquivo oculto. Se não existir, gera uma nova.
    Retorna a chave em base64 url-safe, pronta para uso pelo Fernet.
    """
    ...
    with open(CAMINHO_ARQUIVO_CHAVE, 'rb') as arquivo:
        chave_ofuscada = arquivo.read()
    
    hash_maquina = obter_hash_maquina()
    chave_bytes_puros = bytes([b ^ hash_maquina[i % len(hash_maquina)] for i, b in enumerate(chave_ofuscada)])
    chave_base64 = base64.urlsafe_b64encode(chave_bytes_puros)
    return chave_base64
...
def carregar_credenciais_criptografadas(nome_sistema=""):
    """
    Carrega o login e a senha criptografados. Se a chave estiver corrompida,
    ela é recriada automaticamente.
    Retorna (login, senha) ou (None, None).
    """
    # Tenta carregar a chave e instanciar o Fernet. Se falhar, a chave está corrompida.
    try:
        chave = carregar_chave_criptografia()
        fernet = Fernet(chave)
    except ValueError:
        print("[AVISO] A chave de criptografia está corrompida. Uma nova chave será gerada.")
        if os.path.exists(CAMINHO_ARQUIVO_CHAVE):
            os.remove(CAMINHO_ARQUIVO_CHAVE)
        
        # Tenta novamente, o que irá gerar uma nova chave
        chave = carregar_chave_criptografia()
        fernet = Fernet(chave)

    caminho_credenciais = caminho_arquivo_credenciais("")
    # Se não houver arquivo de credenciais, não há o que carregar.
    if not os.path.exists(caminho_credenciais):
        return None, None

    with open(caminho_credenciais, 'rb') as arquivo:
        dados_criptografados = arquivo.read()

    # Se o arquivo de credenciais estiver vazio, retorna None.
    if not dados_criptografados:
        return None, None

    # Tenta descriptografar as credenciais.
    try:
        dados = fernet.decrypt(dados_criptografados).decode('utf-8')
        credenciais = json.loads(dados)
